...
from users import count_users, delete_user, get_user_ids, get_users
...
job_stores = {
    'default': apscheduler.jobstores.sqlalchemy.SQLAlchemyJobStore(url='sqlite:///jobs.sqlite')
}
...
def get_choose_message_markup(action: str, number):
    ...
    buttons = []
    while i < number:
        buttons.append(InlineKeyboardButton(str(i + 1), callback_data=action + "_message_" + str(i + 1)))
...
async def edit_message(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Выберите сообщение для редактирования:",
                              reply_markup=get_choose_message_markup("edit", number))
...
async def delete_message(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Выберите сообщение для удаления:",
                              reply_markup=get_choose_message_markup("delete", number))
...
async def change_schedule_time(message: Message, state: FSMContext):
    ...
    try:
        if message.text != "0":
            datetime.datetime.strptime(message.text, "%H:%M:%S")
    except:
...
async def _confirm_make_mail(message: Message, state: FSMContext):
    try:
        time = datetime.datetime.strptime(message.text, "%H:%M:%S")
    except:
        await message.answer("Неправильный формат")
        return
...
async def _process_change_kb_end(message: Message, state: FSMContext):
    ...
    try:
        ...
        async with state.proxy() as data:
            new_msg_id = await bot.copy_message(message.from_user.id, message.from_user.id, data["msg_id"],
                                                reply_markup=kb)
            await MailingStates.idle.set()
...
async def _process_mail(call: CallbackQuery, state: FSMContext, callback_data: dict):
    ...
    time: datetime.datetime = data.get("time")
    ...
    mail_thread_on = True
    await make_mail(user_ids, fast, from_user, message, kb, time, has_limit, max_amount, msg, call, all_amount)
...
async def send_mail(bot, user_id: int, from_chat: int, message: Message, kb, time) -> bool:
    try:
        ...
        if time is not None:
            date = datetime.datetime.now() + datetime.timedelta(seconds=time.second, minutes=time.minute,
                                                                hours=time.hour)
...
async def send_message(bot: Bot, send_to, message: UserMessage, title=None, next=None, *next_args):
    ...
    try:
        ...
        if message.Schedule != "0":
            try:
                schedule = datetime.datetime.strptime(message.Schedule, "%H:%M:%S")
            except ValueError:
                await notify_admins(
                    bot,
        ...
        if message.Delete_time != "0":
            try:
                delete_time = datetime.datetime.strptime(message.Delete_time, "%H:%M:%S")
            except ValueError:
                await notify_admins(
                    bot,
        ...
        if schedule != "0":
            ...
            scheduler.add_job(send, trigger=DateTrigger(date, 'Europe/Kiev'))
